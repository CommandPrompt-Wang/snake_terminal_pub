...
构建产物会自动复制 `resources/` 到输出目录。程序启动时会将工作目录切换到可执行文件所在位置，确保 `snake.cfg` 与资源路径正确。
...
游戏设置保存在可执行文件同目录下的 `snake.cfg`（纯文本）。若文件不存在则自动创建默认配置；缺少字段时用默认值补全，正常退出时写回完整文件。
